...
@if($isGeneralPage)
    <h2 class="nav-tab-wrapper">
        <a href="#" data-tab="#tab-gs-scheduling" class="
            ...
            {{ isset($settings[$keyIsSchedulingActive]) && !empty($settings[$keyIsSchedulingActive]) && $settings[$keyIsSchedulingActive][0] ? 'nav-tab-highlight-on' : 'nav-tab-highlight-off' }}
        ">
            {{ _wpcc('Scheduling') }}
        </a>
        ...
        <a href="#" data-tab="#tab-gs-spinning" class="nav-tab">{{ _wpcc('Spinning') }}</a>
        <a href="#" data-tab="#tab-gs-apis" class="nav-tab">{{ _wpcc('APIs') }}</a>
...
@else
    <div class="section-header-button-container">
        <button class="button wpcc-button" id="btn-load-general-settings">{{ _wpcc("Load General Settings") }}</button>
        <button class="button wpcc-button" id="btn-clear-general-settings">{{ _wpcc("Clear General Settings") }}</button>
    </div>
@endif
...
{{-- SPINNING --}}
<div id="tab-gs-spinning" class="tab{{ $isGeneralPage ? '' : '-inside' }} {{ $isGeneralPage ? 'hidden' : '' }}">
    @include('general-settings.tab-spinning')
</div>

{{-- APIs --}}
<div id="tab-gs-apis" class="tab{{ $isGeneralPage ? '' : '-inside' }} {{ $isGeneralPage ? 'hidden' : '' }}">
    @include('general-settings.tab-apis')
</div>
