...
<?php endif; ?>
<table id="ywdpd-table-discounts" class="<?php echo esc_attr( $template ); ?>">
			...
			foreach ( $rules as $rule ) :
				...
				if ( $product->is_type( 'variable' ) ) {
					...
					if ( $prices ) {
						...
						if ( YITH_WC_Dynamic_Pricing_Helper()->$valid_product_function( $main_rule, wc_get_product( $min_key ) ) && ! empty( $min_price ) ) {
						...
						if ( YITH_WC_Dynamic_Pricing_Helper()->$valid_product_function( $main_rule, wc_get_product( $max_key ) ) && ! empty( $max_price ) ) {
					...
					if ( YITH_WC_Dynamic_Pricing_Helper()->$valid_product_function( $main_rule, $product, false ) && ! empty( $price ) ) {
					...
					$discount_price = wc_get_price_to_display( $product, array( 'price' => $discount_price ) );
