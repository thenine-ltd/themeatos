...
if ( ! class_exists( 'YITH_WC_Dynamic_Pricing_Admin' ) ) {
	...
	class YITH_WC_Dynamic_Pricing_Admin {
		...
		public function __construct() {
			...
			// Add action links.
			add_filter( 'plugin_action_links_' . plugin_basename( YITH_YWDPD_DIR . '/' . basename( YITH_YWDPD_FILE ) ), array(
				$this,
				'action_links'
			) );
		...
		public function duplicate_discount() {
			...
			$url_args = array(
				'action' => 'edit',
				'ywdpd_discount_type' => $_GET['ywdpd_discount_type']
			);
			if ( $results ) {
		...
		public function add_custom_type_type( $post ) {
			...
			if ( ywdpd_check_valid_admin_page( $this->post_type_name ) && ! empty( $type ) ) {
				...
				printf( '<a href="%1$s" class="ywpdp_subtitle_link" title="%2$s">%2$s</a>', esc_url( $this->get_panel_page_uri( $tab = $type ) ), esc_html( __( '< Return to Discount List', 'ywdpd' ) ) );
			}
								...
								'rule_type' => array(
									'bulk'              => array(
										...
										'table_note_apply_to'      => array(
											...
											'desc'  => __( 'Enter a custom text to show in the products page where this quantity rule is applied', 'ywdpd' )
										),
									...
									'special_offer'     => array(
										...
										'table_note_apply_to'      => array(
											...
											'desc'  => __( 'Enter a custom text to show in the products page where this special offer is applied', 'ywdpd' )
										),
										'enable_user_rule_exclude' => array(
											'label' => __( 'Exclude users from this discount', 'ywdpd' ),
											'desc'  => __( 'Enable if you want to exclude specific users from this discount', 'ywdpd' )
									...
									'gift_products'     => array(
										...
										'table_note_apply_to'      => array(
											...
											'desc'  => __( 'Enter a custom text to show in the products page where this gift rule is applied', 'ywdpd' )
										),
										'enable_user_rule_exclude' => array(
									...
									'discount_whole'    => array(
										...
										'table_note_apply_to'      => array(
											...
											'desc'  => __( 'Enter a custom text to show in the products page where this rule is applied', 'ywdpd' )
										),
										'enable_user_rule_exclude' => array(
											'label' => __( 'Exclude users from this discount', 'ywdpd' ),
											'desc'  => __( 'Enable if you want to exclude specific users from this discount', 'ywdpd' )
		...
		public function register_panel() {
			...
			if ( defined( 'YITH_YWDPD_FREE_INIT' ) ) {
				...
				$admin_tabs['pricing'] = __( 'Discounts & Price rules', 'ywdpd' );
				$admin_tabs['cart']    = __( 'Cart Rules', 'ywdpd' );
			}
			...
			if ( $enable_shop_manager ) {
				add_filter( 'option_page_capability_yit_' . $args['parent'] . '_options', array(
					$this,
					'change_capability'
				) );
