...
jQuery(function($){
  ...
  //compatibility with x-tab
  $('.woocommerce-tabs ul.x-nav-tabs li a').on('click',function(e){
    var id = $(this).attr('id'),
      tab_content = $('.woocommerce-tabs .x-tab-content').find('div[aria-labelledby="'+id+'"]');

    if( tab_content.find('.ywtm_map').length > 0 ){

      var map = tab_content.find('.ywtm_map');
