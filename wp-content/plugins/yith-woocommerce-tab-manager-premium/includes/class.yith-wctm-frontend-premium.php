...
if ( ! class_exists( 'YITH_WCTM_Frontend_Premium' ) ) {
	class YITH_WCTM_Frontend_Premium extends YITH_WCTM_Frontend {
		...
		public function is_empty( $key ) {
			...
			$type_content = yith_plugin_fw_is_true( $type_content );
		...
		public function customize_woocommerce_tab( $tabs ) {
			...
			foreach ( $tab_type as $type ) {
				...
				if ( $is_hide === 'yes' || 'yes' === $global_hide_option ) {
					...
					$title = apply_filters( 'ywtm_override_wc_tab_title', $product->get_meta( '_ywtm_title_tab_' . $type, true ), $type );
		...
		public function include_style_and_script() {
			...
			if ( is_product() ) {
				...
				wp_register_script(
					'yit-tabmap-script',
					YWTM_ASSETS_URL . 'js/frontend/gmap3.min.js',
					array(
						'jquery',
						'ywtm-google-map',
					),
				...
				if ( ! wp_script_is( 'prettyPhoto' ) || version_compare( WC()->version, '3.0.0', '>=' ) ) {
					...
					wp_register_script(
						'prettyPhoto-init',
						YWTM_ASSETS_URL . 'js/frontend/prettyPhoto/jquery.prettyPhoto.init' . $suffix . '.js',
						array(
							'jquery',
							'prettyPhoto',
						),
