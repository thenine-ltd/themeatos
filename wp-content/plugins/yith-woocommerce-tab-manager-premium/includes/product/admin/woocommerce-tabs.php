	...
	foreach ( $wc_tabs as $key => $tab ) :
		...
		<div id="ywtm_wc_tab_container_<?php echo esc_attr( $key ); ?>" class="options_group ywtm_wc_tab_container">
			<div class="ywtm_wc_tab_option">
				...
				<span class="ywtm_title"><label><?php echo esc_attr( $tab['title'] ); ?></label></span>
				  <span class="ywtm_hide">
					<label><?php esc_html_e( 'Hide', 'yith-wc-tab-manager' ); ?></label>
					<input type="checkbox" class="checkbox ywtm_hide_cb" name="ywtm_hide_<?php echo esc_attr( $key ); ?>" <?php checked( 'yes', $is_hide ); ?> data-tab_type="<?php echo esc_attr( $key ); ?>"/>
				...
				<span class="ywtm_override">
					<label><?php esc_html_e( 'Override', 'yith-wc-tab-manager' ); ?></label>
						<input type="checkbox" class="checkbox ywtm_override_cb" name="ywtm_override_<?php echo esc_attr( $key ); ?>" <?php checked( 'yes', $is_override ); ?>  data-tab_type="<?php echo esc_attr( $key ); ?>"/>
			...
			<div class="ywtm_wc_tab_content" id="ywtm_wc_tab_content_<?php echo esc_attr( $key ); ?>">
				<p class="form-field">
					<label><?php esc_html_e( 'Tab Title', 'yith-woocommerce-tab-manager' ); ?></label>
					<input type="text" name="ywtm_title_tab_<?php echo esc_attr( $key ); ?>" class="ywtm_title_tab_<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $tab_title ); ?>" />
					<?php if ( $key === 'reviews' ) : ?>
				...
				switch ( $key ) {
					case 'description':
						?>
							 <p class="form-field">
								 <label><?php esc_html_e( 'Tab Content', 'yith-woocommerce-tab-manager' ); ?></label>
							 <?php
				...
				?>
				<p class="form-field">
					<label><?php esc_html_e( 'Priority', 'yith-woocommerce-tab-manager' ); ?></label>
					<input type="number" min="0" name="ywtm_priority_tab_<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $tab_priority ); ?>">
				</p>
			</div>
		</div>
   <?php endforeach; ?>
