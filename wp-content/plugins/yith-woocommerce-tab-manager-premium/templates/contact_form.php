...
<div class="yit_wc_tab_manager_contact_form_container ywtm_content_tab">
	...
	<form class="ywtm_contact_form" method="post">
		<fieldset>
			<div class="primary_contact_information">
				<?php if ( $show_name ) : ?>
					<div class="contact_name_field <?php echo esc_attr( $col ); ?> contact_field">
						<input type="text" name="ywtm_name_contact_field"
							   placeholder="<?php esc_attr_e( 'Your name', 'yith-woocommerce-tab-manager' ); ?><?php echo esc_html( $star_name ); ?>"/>
				...
				<div class="contact_email_field <?php echo esc_attr( $col ); ?> contact_field">
					<input type="text" name="ywtm_email_contact_field"
						   placeholder="<?php esc_html_e( 'Email', 'yith-woocommerce-tab-manager' ); ?>*"/>
					<input type="hidden" name="ywtm_req_email" value="req"/>
				</div>

				<?php if ( $show_webaddr ) : ?>
					<div class="contact_webaddr_field <?php echo esc_attr( $col ); ?> contact_field">
						<input type="text" name="ywtm_webaddr_contact_field"
							   placeholder="<?php esc_attr_e( 'Website', 'yith-woocommerce-tab-manager' ); ?><?php echo esc_attr( $star_addr ); ?>"/>
			...
			<div class="secondary_contact_information">
				<?php if ( $show_subject ) : ?>
					<div class="contact_subj_field ywtm_col_1">
						<input type="text" name="ywtm_subj_contact_field"
							   placeholder="<?php esc_attr_e( 'Subject', 'yith-woocommerce-tab-manager' ); ?><?php echo esc_attr( $star_subj ); ?>"/>
				...
				<div class="contact_textarea_field ywtm_col_1">
					<textarea name="ywtm_info_contact_field"
							  placeholder="<?php esc_attr_e( 'Your Message', 'yith-woocommerce-tab-manager' ); ?>*"></textarea>
					<input type="hidden" name="ywtm_req_info" value="req"/>
					<input type="hidden" name="ywtm_product_id" value="<?php echo esc_attr( $product_id ); ?>"/>
					<input type="hidden" name="ywtm_action" value="ywtm_sendermail"/>
					<?php
					if ( defined( 'ICL_LANGUAGE_CODE' ) ) {
						echo '<input type="hidden" name ="ywtm_language" value="' . ICL_LANGUAGE_CODE . '">'; //phpcs:ignore WordPress.Security.EscapeOutput
					}
					?>
					...
					<span id="ywtm_btn_container"><input type="submit" class="ywtm_btn_sendmail"
														 value="<?php esc_attr_e( 'Send', 'yith-woocommerce-tab-manager' ); ?>"/></span>
	...
	<span>*<?php esc_html_e( 'required', 'yith-woocommerce-tab-manager' ); ?></span>
</div>
