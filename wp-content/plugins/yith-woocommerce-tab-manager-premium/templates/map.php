...
<div class="google-map ywtm_content_tab">
	<div class="gmap3 ywtm_map"   style="height:<?php echo esc_attr( $heig ); ?>px;<?php echo esc_attr( $css_width ); ?>" data-zoom="<?php echo esc_attr( $zoom ); ?>" data-address="<?php echo esc_attr ( $address ); ?>"></div>
	<a class="link_google_map" target="_blank" href="<?php echo esc_url( add_query_arg( array( 'q' => urlencode( $addr ) ), '//maps.google.com/' ) ); ?>"><?php esc_html_e( 'Show in Google Map', 'yith-woocommerce-tab-manager' ); ?></a>
</div>
<?php } else {
	echo '<p>' . esc_html__( 'No map for this product', 'yith-woocommerce-tab-manager' ) . '</p>';
}
?>
